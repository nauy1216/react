...
(function (global, factory) {
  ...
  var UserBlockingPriority$1 = unstable_UserBlockingPriority,
      runWithPriority$1 = unstable_runWithPriority; // TODO: can we stop exporting these?
  ...
  function updateDOMProperties(domElement, updatePayload, wasCustomComponentTag, isCustomComponentTag) {
    debugger; // TODO: Handle wasCustomComponentTag
  ...
  function flushPassiveEffectsImpl() {
    ...
    for (var i = 0; i < unmountEffects.length; i += 2) {
      ...
      var _fiber = unmountEffects[i + 1];
      var destroy = _effect.destroy;
      _effect.destroy = undefined;

      {
        _fiber.flags &= ~PassiveUnmountPendingDev;
    ...
    for (var _i = 0; _i < mountEffects.length; _i += 2) {
      ...
      var _fiber2 = mountEffects[_i + 1];

      {
        setCurrentFiber(_fiber2);
        ...
        if (hasCaughtError()) {
          ...
          captureCommitPhaseError(_fiber2, _error4);
        }
  ...
  function createRootImpl(container, tag, options) {
    ...
    var root = createContainer(container, tag, hydrate); // 在DOM元素上添加__reactContainer$属性， 用来标记这是container元素

    markContainerAsRoot(root.current, container);
    var rootContainerElement = container.nodeType === COMMENT_NODE ? container.parentNode : container; // 在根节点上进行事件代理
