...
const IsSomeRendererActing = ReactSharedInternals.IsSomeRendererActing;
const NO_CONTEXT = {};
...
function createReactNoop(reconciler, useMutation) {
  ...
  const sharedHostConfig = {
    ...
    getFundamentalComponentInstance(fundamentalInstance) {
      const impl = fundamentalInstance.impl,
  ...
  const hostConfig = useMutation ? _assign({}, sharedHostConfig, {
    ...
    cloneHiddenInstance(instance, type, props, internalInstanceHandle) {
      const clone = cloneInstance(instance, null, type, props, props, internalInstanceHandle, true);
      clone.hidden = true;
      return clone;
    },
  ...
  const ReactNoop = {
    ...
    createRoot() {
      ...
      const fiberRoot = NoopRenderer.createContainer(container, ReactRootTags.ConcurrentRoot, false, null);
      return {
        _Scheduler: Scheduler,
    ...
    createBlockingRoot() {
      ...
      const fiberRoot = NoopRenderer.createContainer(container, ReactRootTags.BlockingRoot, false, null);
      return {
        _Scheduler: Scheduler,
    ...
    createLegacyRoot() {
      ...
      const fiberRoot = NoopRenderer.createContainer(container, ReactRootTags.LegacyRoot, false, null);
      return {
        _Scheduler: Scheduler,
    ...
    renderLegacySyncRoot(element, callback) {
      ...
      const container = ReactNoop.getOrCreateRootContainer(rootID, ReactRootTags.LegacyRoot);
      const root = roots.get(container.rootID);
      NoopRenderer.updateContainer(element, root, null, callback);
    },

    renderToRootWithID(element, rootID, callback) {
      const container = ReactNoop.getOrCreateRootContainer(rootID, ReactRootTags.ConcurrentRoot);
      const root = roots.get(container.rootID);
      NoopRenderer.updateContainer(element, root, null, callback);
    },
  ...
  const batchedUpdates = NoopRenderer.batchedUpdates,
...
const _createReactNoop = createReactNoop(ReactFiberReconciler, // reconciler
false // useMutation
),
