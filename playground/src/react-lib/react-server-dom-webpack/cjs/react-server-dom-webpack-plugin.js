...
class ReactFlightWebpackPlugin {
  ...
  apply(compiler) {
    ...
    var run = function (params, callback) {
      ...
      _this.resolveAllClientFiles(compiler.context, contextResolver, compiler.inputFileSystem, compiler.createContextModuleFactory(), function (err, resolvedClientReferences) {
        if (err) {
          callback(err);
          return;
        }
