...
const PerformedWork =
/*                */
0b00000000000000000001; // You can change the rest (and add more).
...
/*               */
0b00000001111111111111; // These are not really side effects, but we still reuse this field.
...
const ShouldCapture =
/*                */
0b00000100000000000000; // TODO (effects) Remove this bit once the new reconciler is synced to the old.
const ForceUpdateForLegacySuspense =
/* */
0b00010000000000000000; // Static tags describe aspects of a fiber that are not specific to a render,
...
const PassiveStatic =
/*                */
0b00100000000000000000; // These flags allow us to traverse to fibers that have effects on mount
...
const getFundamentalComponentInstance = $$$hostConfig.getFundamentalComponentInstance;
const mountFundamentalComponent = $$$hostConfig.mountFundamentalComponent;
const shouldUpdateFundamentalComponent = $$$hostConfig.shouldUpdateFundamentalComponent;
const getInstanceFromNode = $$$hostConfig.getInstanceFromNode;
const isOpaqueHydratingObject = $$$hostConfig.isOpaqueHydratingObject;
const makeOpaqueHydratingObject = $$$hostConfig.makeOpaqueHydratingObject;
const makeClientId = $$$hostConfig.makeClientId;
const makeClientIdInDEV = $$$hostConfig.makeClientIdInDEV;
...
const prepareScopeUpdate = $$$hostConfig.preparePortalMount;
...
const updateFundamentalComponent = $$$hostConfig.updateFundamentalComponent;
const unmountFundamentalComponent = $$$hostConfig.unmountFundamentalComponent;
...
const cloneFundamentalInstance = $$$hostConfig.cloneInstance; // -------------------
//     Hydration
//     (optional)
// -------------------
...
const didNotHydrateContainerInstance = $$$hostConfig.didNotHydrateContainerInstance;
const didNotHydrateInstance = $$$hostConfig.didNotHydrateInstance;
const didNotFindHydratableContainerInstance = $$$hostConfig.didNotFindHydratableContainerInstance;
const didNotFindHydratableContainerTextInstance = $$$hostConfig.didNotFindHydratableContainerTextInstance;
const didNotFindHydratableContainerSuspenseInstance = $$$hostConfig.didNotFindHydratableContainerSuspenseInstance;
...
const now$1 = initialTimeMs < 10000 ? Scheduler_now : () => Scheduler_now() - initialTimeMs;
...
function ChildReconciler(shouldTrackSideEffects) {
  ...
  function reconcileSingleElement(returnFiber, currentFirstChild, element, lanes) {
    ...
    if (element.type === REACT_FRAGMENT_TYPE) {
      ...
      // 通过 element 创建fiber
      const created = createFiberFromElement(element, returnFiber.mode, lanes); // 设置ref
...
const now$2 = Scheduler.unstable_now;
let commitTime = 0;
...
function stopProfilerTimerIfRunningAndRecordDelta(fiber, overrideBaseTime) {
  ...
  if (profilerStartTime >= 0) {
    const elapsedTime = now$2() - profilerStartTime;
...
function updateHostRoot(current, workInProgress, renderLanes) {
  ...
  const nextProps = workInProgress.pendingProps; // memoizedState上保存了旧的props

  const prevState = workInProgress.memoizedState; // prevState.element保存了children

  const prevChildren = prevState !== null ? prevState.element : null; // 如果current和workInProgress的updateQueue是同一个对象
  ...
  if (value !== null && typeof value === 'object' && typeof value.then === 'function') {
    ...
    do {
      if (workInProgress.tag === SuspenseComponent && shouldCaptureSuspense(workInProgress, hasInvisibleParentBoundary)) {
        ...
        if ((workInProgress.mode & BlockingMode) === NoMode && workInProgress !== returnFiber) {
          workInProgress.flags |= DidCapture;
      ...
      case Deletion:
        {
          ...
          if (deletions !== null) {
            for (let i = 0; i < deletions.length; i++) {
              const deletion = deletions[i]; // Clear the deletion effect so that we don't delete this node more
              // than once.
...
function updateContainer(element, container, parentComponent, callback) {
  ...
  const current = container.current; // 获取一个时间， 做什么用？
